...
void naive_bubble(int *dynamic, int iter) {
  ...
  {
    for(int i=0;i<iter-1;i++)   // compare those elements of the array
    {
      if(dynamic[i]>dynamic[i+1])
      {
        dynamic[i]=dynamic[i]+dynamic[i+1];     // Used swap here !
	...
	//TODO -> Create an optimized version of bubblesort here that will sort dynamic
...
void main() {
	...
	for (int k = 0; k < (sizeof(num_of_array_ele) / sizeof(num_of_array_ele[0])); k++) {
                dynam_array = (int*)malloc(num_of_array_ele[k]* sizeof(int)); //used malloc to dynamically allocate memory !!
